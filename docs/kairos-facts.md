...
| パネル | AT-KC10C1G（デッキ TOP / BOTTOM は同じ Core の 2 系統として使う） |
| MV | DisplayPort × 4 |

- SDI 出力 16 本の割り当てはこのアプリの CONFIG > OUTPUT が持つ。
  ボード単位（B1 = OUT 1-4、B2 = 5-8、B3 = 9-12、B4 = 13-16）で並べている
- デュアル Core を使わないので、パネルの Deck A / Deck B は
  **同一 Core の異なるシーンへのデリゲート**として扱う（Core-1/2 の切り替えは出さない）
...
6. ~~SDI ボード枚数と実入出力数~~ → **32 IN / 16 OUT、ボード 4 枚フル。確定**
7. ~~デュアル Core を使うか~~ → **使わない。確定**
8. SDI 入力 32 本の実際の使用本数とパッチ（現在はサンプルで 6 本）
9. SDI 出力 13-16（予備 4 本）の用途
